...
public class BbsDAO {
	private static BbsDAO instance = new BbsDAO(); // 자기 자신으로 객체 형성 static 모든
													// 문서에 전반적으로 영향을 미침.
	...
	// 커넥션
	public Connection getConnection() throws ClassNotFoundException, SQLException { // type을
																					// Connection으로
																					// 바꿈
	...
	public List<BbsDBBean> getMembers() {
		...
		try {
			...
			// sql 전체회원 가져오기
			String sql = "select number,title,name,date,hit from anony order by number desc";
			pstmt = conn.prepareStatement(sql);
	...
	public BbsDBBean getBbs(int number) {
		...
		try {
			...
			if (rs.next()) {
				...
				mb.setDate(rs.getTimestamp("date"));
				int bbsCount = rs.getInt("hit");

				mb.setHit(bbsCount);
				bbsCount++;
				countUpdate(bbsCount, number);
				return mb;// 6개의 항목을 bbs인스턴스에 넣어 반환한다.
	...
	public int countUpdate(int bbsCount, int bbsID) {
		Connection conn = null;
		String SQL = "update anony set hit = ? where number = ?";
		try {
